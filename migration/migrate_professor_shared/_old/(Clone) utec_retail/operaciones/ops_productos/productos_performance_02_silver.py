...
# dbutils.widgets.text('GOLD_VW_INVENTARIO', f'{my_group}_ops_productos.gold.vw_inventario')
# dbutils.widgets.text('GOLD_VW_VENTAS', f'{my_group}_cmc_ventas.gold.vw_ventas')
# dbutils.widgets.text('SILVER_TB_UM_PROD_PERF', f'{my_group}_ops_productos.silver.um_producto_performance')
# dbutils.widgets.text('SILVER_TB_HM_PROD_PERF', f'{my_group}_ops_productos.silver.hm_producto_performance')
...
spark.sql(
    f"""
    CREATE OR REPLACE TABLE {SILVER_TB_UM_PROD_PERF}
    AS
    SELECT
        periodo,
        producto_id,
